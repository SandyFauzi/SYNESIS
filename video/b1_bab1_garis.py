...
class B1Bab1(Scene):
    def construct(self):
        ...
        jd = judul("Garis Tidak Cukup", 34)
        sj = subjudul(["Bulan 1 Sesi 2  ·  apa yang bertambah",
                       "ketika satu lapisan dipasang"])
        mk, kk = merek(), kaki()
        ...
        self.play(LaggedStart(*[GrowFromCenter(d) for d in merah],
                  ...
                  LaggedStart(*[GrowFromCenter(d) for d in biru],
                              lag_ratio=0.006, run_time=0.9))
        ket = label_kecil("dua kelompok yang saling mengunci",
        ...
        cacah = pencacah(f"satu garis lurus   akurasi {AK_LIN * 100:.1f} persen",
                         PALET["jingga"], 19)
        self.play(FadeIn(cacah, run_time=0.4))
        ket2 = label_kecil("apa pun sudutnya, selalu ada yang salah sisi",
                           PALET["jingga"], 16).next_to(sumbu, DOWN, buff=0.20)
        self.play(ReplacementTransform(ket, ket2), run_time=0.5)
        self.wait(1.4)

        # ---------- pasang satu lapisan ----------
        sj2 = subjudul([f"pasang {N_SEMBUNYI} neuron ReLU di tengah.",
                        "tiap satu menyumbang satu lipatan"])
        ...
        ket3 = label_kecil("tiap garis: tempat satu neuron menyala atau padam",
        ...
        rket = label_kecil("satu tekukan. bukan kurva, cuma satu patahan.",
                           PALET["redup"], 15)
        ...
        cacah2 = pencacah(
            f"{N_SEMBUNYI} neuron tersembunyi   akurasi {AK_MLP * 100:.1f} persen",
            PALET["hijau"], 19)
        self.play(FadeIn(peta_m, run_time=1.0),
                  garis_lipat.animate.set_stroke(opacity=0.22),
                  Transform(cacah, cacah2), run_time=1.0)

        ket4 = label_kecil("batasnya melengkung, karena disusun dari patahan lurus",
                           PALET["hijau"], 15).next_to(sumbu, DOWN, buff=0.20)
        self.play(ReplacementTransform(ket3, ket4), run_time=0.5)
        self.wait(1.6)

        kartu = kartu_angka([
            ("garis lurus", f"{AK_LIN * 100:.2f} persen"),
            (f"{N_SEMBUNYI} neuron ReLU", f"{AK_MLP * 100:.2f} persen"),
        ...
        sj3 = subjudul(["harganya: titik awal sekarang",
                        "menentukan tempat mendarat"])
        ...
        lbl = label_kecil("akurasi dari delapan titik awal acak. "
                          "model dan data sama persis.",
                          PALET["redup"], 14).next_to(sumbu2, DOWN, buff=0.20)
        ...
        kartu2 = kartu_angka([
            ("akurasi terbaik", f"{max(AK_SURVEI) * 100:.2f} persen"),
            ("akurasi terburuk", f"{min(AK_SURVEI) * 100:.2f} persen"),
            ("titik awal yang nyangkut", f"{n_gagal} dari {len(AK_SURVEI)}"),
        ], ukuran=17, warna_nilai=PALET["ungu"]).move_to([0, -3.9, 0])
        self.play(FadeIn(kartu2, shift=UP * 0.2, run_time=0.6))
        self.wait(1.5)

        tutup = Paragraph(
            "Di Bulan 0 mangkuknya tunggal,",
            "jadi dari mana pun kamu mulai, tiba di dasar yang sama.",
            "Mulai Bulan 1 itu berhenti berlaku.",
